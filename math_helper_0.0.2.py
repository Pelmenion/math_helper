...
def geometry():
    geometry_page = Toplevel(home_page)
    geometry_page.title('Geometry')
    geometry_page.geometry('200x150')

    def geometry1():
        geometry1 = Toplevel(geometry_page)
        var_combobox = ttk.Combobox(geometry1, values=["Выберете действие","Найти С", "Найти А или В"])
        var_combobox.current(0)
        # def geometry1_choose():
            # combobox= var_combobox.get()
            # geometry11 = Toplevel(geometry_page)
            # geometry11.geometry('200x200')
            #
            # if combobox == "Найти С":
            #     geometry11.title('Найти С')
            #
            #
            # elif combobox == "Найти А или В":
            #     geometry11.title('Найти А или В')
            #     label_frame_a_b = LabelFrame(geometry11, text='Введите известные данные')

        bttn_geometry1_choose = Button(geometry1, text='Выбрать', command=comming_soon)

    label_frame_geometry = LabelFrame(geometry_page, text="Выберите:")
    label_frame_geometry.pack(padx=5, pady=5)

    bttn_geometry1 = Button(label_frame_geometry, text='Теорема Пифагора', command=comming_soon)
    bttn_geometry1.pack(padx=5, pady=5)

    bttn_geometry2 = Button(label_frame_geometry, text='Высота фигур', command=comming_soon)
    bttn_geometry2.pack(padx=5, pady=5)

    bttn_geometry3 = Button(label_frame_geometry, text="Площадь фигур", command=comming_soon)
    bttn_geometry3.pack(padx=5, pady=5)
